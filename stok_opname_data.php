...
// require_once('array2json.php');
 ...
 @$qty=$_POST['q'];
 @$jenis=$_POST['j'];
 @$tp_gudang=$_POST['tg'];//tipe gudang terdiri dari o outlet,m=markas,g=distribusi
 ...
 if($jenis=='otl'){// hitung bruto menghasilkan 2 variable qty bruto dan nilai penjualan bruto  
	...
	*/
	...
	if(mysql_affected_rows()<=0){ 
		   $sql_otl="select trim(nama),jenis from  outlet where id='$gudang'";
...
VALUES ('$gudang', '$tgl', '$username',now());";
	...
	if($jumlah_koreksi!==''){
	 ...
	 $sql="SELECT  SQL_CACHE sp.kd_".$table." as id, SUM(sp.stok_awal) as stokawal , SUM(sp.stok_awal * $script_hpp) as hpp , SUM(sp.stok_awal  * p.hargajual ) as hpj 
	  FROM ".$table_koreksi_stok." AS sp  LEFT JOIN  produk p ON (p.kode = sp.kode_15)
	 WHERE sp.tanggal like '$tgl%' $sql_tbhn  GROUP BY sp.tanggal,sp.kd_".$table;
	...
	if($jumlah_koreksi!==''){
	 ...
	 $sql=" SELECT  UPPER(k.kd_".$table.") AS id,SUM(k.stok_akhir) AS so,SUM(k.stok_akhir * ".$script_hpp.") AS hpp , SUM(k.stok_akhir * p.hargajual) AS hpj FROM produk AS p
      RIGHT JOIN ".$table_koreksi_stok." AS k ON (p.kode = k.kode_15)
      WHERE k.tanggal LIKE '$tgl%'  $sql_tbhn  GROUP BY k.kd_".$table." ;";
	 //die($sql);
	 if($username=='budi-it'){
			$sql_debug=mysql_escape_string($sql);
	...
	if($d[0].'-'.$d[1]!=date('Y-m')){
     ...
     WHERE k.kd_".$table."='$gudang' AND k.tanggal LIKE '$tgl' $filter ;";
...
FROM `rekap_stok_outlet$monthTahun` AS `r` INNER JOIN `produk` AS `p`      ON (`r`.`barcode_15` = `p`.`kode`)
	...
	$sql=" select SQL_CACHE '$tgl' as bulan,kode_".$table." as id, sum( $fields ) as stokawal, sum( ($fields) * $script_hpp ) as hpp,sum( ($fields) * p.hargajual ) as hpj  from rekap_stok_".$table.$tbhn." 
	       r INNER JOIN produk p ON r.barcode_15=p.kode where   r.periode like '$periode%'  group by r.kode_".$table." ";
	#die($sql);
