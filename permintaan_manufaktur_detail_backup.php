...
function simpan(){
	...
	$.post("manufaktur_query.php",data,function(response){
		...
		if(response.trim()=='ada'){
			alert('Barcode telah di pakai');exit();
		}else if(response.trim()=='no'){
			alert('barcode tidak di temukan di master');exit();
		}else{
		...
		<table width="100%">
		  <tr>
				<td align="center">
					<?php 
						if($approved){
							...
							if($approved2){
								...
								if($rmkurang){
									?> <input type="button" value="Daftar Kekurangan RM" onclick="window.location='rm_kurang_list.php?no_manufaktur=<?php echo $no_manufaktur; ?>';"> <?php
								}
							}else{
								?><input type="button" value="Approve II" onclick="if(confirm('Approving Manufaktur <?php echo $no_po; ?>?')){window.location='po_manufaktur_approving2.php?no_manufaktur=<?php echo $no_manufaktur; ?>';}"> <?php
							}
						}else{
							?><input type="button" value="Approve I" onclick="if(confirm('Approving Manufaktur <?php echo $no_po; ?>?')){window.location='po_manufaktur_approving.php?no_manufaktur=<?php echo $no_manufaktur; ?>';}"> <?php
						}
					?>
					<?php						
						if($closeco=="1"){
					...
					<?php
						if($approved){
							?><input type="button" value="Print Mode" onclick="window.open('permintaan_manufaktur_print.php?no_manufaktur=<?php echo $no_manufaktur; ?>','permintaan_manufaktur_print','width=800,height=400,menubar=yes,scrollbars=yes');"><?php
						}
					?>
