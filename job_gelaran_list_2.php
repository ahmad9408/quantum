...
?>
	...
	<fieldset style="width=1%">
        ...
        <form id="gelaran" name="gelaran" method="post" action="job_gelaran_list_2.php?action=search">	
		<table width="968">
			<tr>
				...
				<td width="361">Pencarian <input name="model" type="text" id="model" value="<?php echo $kode_md;?>"  size="30" /></td>
		...
		<?php
			...
			if(isset($_REQUEST['action'])){
			   ...
			   /* $sql="SELECT  SQL_CALC_FOUND_ROWS  j.seqno , j.no_po, j.no_co, j.pabrik , j.updateby , j.updatedate, j.approve , j.approvedate, j.approveby  FROM job_gelaran_detail AS d
					...
					WHERE (m.model LIKE '%$kode_md%' or j.no_po like '$kode_md%' or j.no_co like '$kode_md%')    group by j.no_co
...
$sql="SELECT
 DISTINCT(`job_gelaran`.`no_co`)
    ...
    , `pabrik`.`nama`
FROM
    `quantum`.`job_gelaran_detail`
    INNER JOIN `quantum`.`job_gelaran` 
        ON (`job_gelaran_detail`.`no_co` = `job_gelaran`.`no_co`)
    INNER JOIN `quantum`.`pabrik` 
        ON (`job_gelaran`.`pabrik` = `pabrik`.`id`)
    INNER JOIN `quantum`.`produk` 
        ON (`job_gelaran_detail`.`kd_produk` = `produk`.`kode`) where (`job_gelaran`.`no_po` like '%$kode_md%'
		or `job_gelaran`.`no_co` like '%$kode_md%' or `pabrik`.`nama` like '%$kode_md%' or produk.nama like '%$kode_md%')  GROUP BY job_gelaran.no_co 
        ORDER BY approve,updatedate DESC,no_po LIMIT  ".($page*$jmlHal).",".$jmlHal; */
...
FROM
    ...
    INNER JOIN `quantum`.`pabrik` 
		...
		or `job_gelaran`.`no_co` like '%$kode_md%' or `pabrik`.`nama` like '%$kode_md%' or job_gelaran.model like '%$kode_md%') ORDER BY 
        `job_gelaran`.`approve`, `job_gelaran`.`updatedate` desc LIMIT  ".($page*$jmlHal).",".$jmlHal;
}else{
			  $sql="SELECT   no_po,no_co,pabrik, updateby, updatedate, approve, approveby, approvedate FROM job_gelaran WHERE no_po LIKE '%$no_po' AND pabrik LIKE '$_pabrik%' ORDER BY approve,updatedate DESC,no_po limit ".($page*$jmlHal).",".$jmlHal;
}
...
// Edited Bye Goberan
	...
	if($approved){
        ...
        <td align="center" width="87" height="20"><a href="job_gelaran_detail_2.php?no_po=<?php echo $no_po; ?>&no_co=<?php echo $no_co; ?>">Detil</a></td>
    </tr>
		<?php
			}
		?>
