...
<table border="1" width="100%" style="font-size: 10pt" cellspacing="0" cellpadding="0" bordercolor="#C0C0C0">
	...
	$sql = "SELECT $sql_cache * FROM job_loading WHERE pabrik_dari=pabrik_tujuan AND pabrik_dari LIKE '$_pabrik%' AND approve2='1' ORDER BY tanggal DESC,no_load";
		  ...
		  FROM
			...
			LEFT JOIN `quantum`.`job_gelaran` AS `jg`
			  ON (`jl`.`no_co` = `jg`.`no_co`)
			   WHERE pabrik_dari=pabrik_tujuan AND `jl`.`tanggal` > DATE('2022-01-01 00:00:00') $terusan_pabrik AND  approve2='1' and (`no_load` like '%$cari%' or `nama` like '%$cari%') ORDER BY tanggal DESC,no_load ";
	...
	while ($rs = mysql_fetch_array($hsl)) {
		...
		$sql = "SELECT $sql_cache SUM(jsd.qty-jsd.pending-jsd.reject) FROM job_sewing_detail AS jsd 
			...
			<?php for ($i = 0; $i < ($jmlData / $jmlHal); $i++) {
				if ($hal <= 0) { ?>
					<span class="<?php if ($i == $hal) echo "hal_select";
									else echo "hal"; ?>" onclick="location.href='window_job_loading.php?hal=<?php echo $i ?>&textid=no_load&cari=<?php echo $cari ?>';"><?php echo ($i + 1); ?></span>
					<?php if ($i >= 4) break;
