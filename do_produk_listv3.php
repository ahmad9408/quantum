<?php 
	...
	edit budi tgl 27 maret 2015
	pencarian model bisa menghasilkan beberapa kode ,sebelumnya hanya menghasilkan 1 jenis barcode saja
	jika pencarian berdasarkan master tidak ada maka tidak diikut sertakan
	
	edit budi tgl 19 jun 2015
	perubahan tujuan
	
	edit budi tambah base
	
	edit terakhir 17112016 detail ganti 
	
	v3 tambah piliahn kode/barcode 10062022 hindari cache
	*/
...
if(!empty($_REQUEST['hal'])){
	...
	$gudang_filter=$_POST['gudang_filter'];
    ...
    <form action="?search=yes&rnd=<?php  echo date('YmdHis'); ?>" method="POST" name="f1">
		...
		<table>
			<tr>
			  <td>Barcode</td>
			  <td>&nbsp;</td>
			  <td><input type="text" id="txt_kode" name="txt_kode" size="20" value="<?php echo $txt_kode;?>" /></td>
			  <td>&nbsp;</td>
			  <td>&nbsp;</td>
			  <td align="right">&nbsp;</td>
			  <td>&nbsp;</td>
			...
			<tr>
				<td><b> nama Model </b></td>
				<td><b>:</b></td>
				
				  <td><input type="text" id="search_model2" name="search_model" size="20" value="<?php echo $search_model?>" /></td>
				  <td width="10">&nbsp;</td>
				<td>Cari No Do/Tanggal/Pabrik<input type="text" id="search_model" name="master" size="20" value="<?php echo $_POST[master]?>">&nbsp;&nbsp;&nbsp;
				
				 <script language="JavaScript" src="calendar_us.js"></script>
            <link rel="stylesheet" href="calendar.css">
            </td><td align="right">Pilih :</td> <td>
			 ...
			 ?>
            <select id="pilst" name="pilst">
            <option value="0" <?php if($pilst==0){echo"selected";}?>>All</option>
            <option value="1" <?php if($pilst==1){echo"selected";}?>>Sudah Approve 2</option>
            <option value="2" <?php if($pilst==2){echo"selected";}?>>Belum Approve 2</option>
            </select> &nbsp;jml hal : <input type="text" size="3" id="jmlh" name="jmlh" value="<?php echo $jmlh?>" />
			&nbsp;</td>
            
			<input type="hidden" id="tg" name="tg" value="<?php echo $_GET['tgl1'];?>" />
			<input type="hidden" id="hal" name="hal" /> 
					</td>
				<td><input type="submit" id="but" value="Search"></td>
			</tr>
			<tr>
			  <td><strong>Berdasarkan</strong></td>
			  <td><strong>:</strong></td>
			  <td><select id="berdasarkan" name="berdasarkan">
              <?php
			     foreach($base as $id=>$value){
					 ?>
                     <option value="<?php echo $id; ?>" <?php if($id==$berdasarkan){echo"selected";}?>><?php echo $value; ?>&nbsp;</option>
                  <?php   
			     }
			  ?>
...
<script language="JavaScript">
			  ...
			  <td><?php  $sql="select $sql_cache id,nama from gudang_distribusi where jenis=1";
			...
			?>
		        ...
		        <option value="0">Pilih All</option>
			      <?php while(list($id_gudang,$nama_gudang)=mysql_fetch_array($res)){
					?>
			      <option value="<?php echo $id_gudang?>" <?php if($gudang_filter==$id_gudang){ echo 'selected="selected"';}?> ><?php echo $nama_gudang?></option>
			      <?php
				}?>
	          </select></td>
			  <td>              
	...
	?>
	<table  border="1" width="1184" style="font-size: 10pt" height="122" cellspacing="0" cellpadding="0" bordercolor="#ECE9D8">
		<tr>
            ...
            <td align="center" width="20" bgcolor="#99CC00" height="20"><b>Kode Pabrik</b></td>
            <td align="center" width="20" bgcolor="#99CC00"><strong>Kode Tujuan</strong></td>
            <td align="center" width="20" bgcolor="#99CC00">Tujuan</td>
			<td align="center" width="20" bgcolor="#99CC00" height="20"><b>Total Qty</b></td>
			<td align="center" width="20" bgcolor="#99CC00" height="20"><b>Jml Hpp (Rp)</b></td>
			...
			<td align="center" width="20" bgcolor="#99CC00" height="20"><b>Jml Hari</b></td>
			<td  width="20" bgcolor="#99CC00" height="20" align="center"><b>Action</b></td>
		</tr>
		<?php
			// Edit By Goberan <-> 26-6-2010
			if(isset($_REQUEST['hal'])) $hal=$_REQUEST['hal']; else $hal=0;
			$jmlHal=$jmlh;
			$page=$hal;
			if($_SESSION["id_group"]=="1" || $typeoutlet=="G"){
				if ( $_GET[search] == "yes") {
					...
					//do_produk.no_do NOT LIKE 'P100S%' and
						...
						$sql="SELECT SQL_CACHE do_produk.*,pabrik.nama,DATEDIFF('" .date("Y-m-d") ."',tanggal) as usia FROM do_produk,do_produk_detail,pabrik WHERE  do_produk.no_do NOT LIKE '%smt%' and do_produk.no_do NOT LIKE '%btl%'  and  do_produk.no_fin LIKE '%$no_fin' and do_produk.no_do=do_produk_detail.no_do and do_produk_detail.kd_produk LIKE '$barcode%' and pabrik.id=substring(do_produk.no_do,1,5)
						 $sql_tbhn $addcode $terapp group by do_produk.no_do   ORDER BY approve,tanggal DESC,no_do DESC ";
					...
					//no_do NOT LIKE 'P100S%' and
					   if(!empty($gudang_filter)){
						...
						$sql="SELECT SQL_CACHE do_produk.*,pabrik.nama,DATEDIFF('" .date("Y-m-d") ."',tanggal) as usia FROM do_produk,pabrik WHERE  no_do NOT LIKE '%smt%' and no_do NOT LIKE '%btl%' and  no_fin LIKE '%$no_fin'   and (no_do like '$_POST[master]%' or tanggal like '$_POST[master]%' or pabrik.nama like '%$_POST[master]%') $sql_tbhn $addcode $terapp  group by do_produk.no_do ORDER BY approve,tanggal DESC,no_do DESC ";
					}
				} else {
				//no_do NOT LIKE 'P100S%' and 
					$sql="SELECT SQL_CACHE do_produk.*,pabrik.nama,DATEDIFF('" .date("Y-m-d") ."',tanggal) as usia FROM do_produk,pabrik WHERE no_do NOT LIKE '%smt%' and no_do NOT LIKE '%btl%' and no_do NOT LIKE '%btl%' and  no_fin LIKE '%$no_fin'   and (no_do like '$_POST[search_model]%' or tanggal like '$_POST[search_model]%' or pabrik.nama like '%$_POST[master]%' )  $addcode  $terapp group by do_produk.no_do ORDER BY approve,tanggal DESC,no_do DESC ";
				}
				...
				if ( $_GET[search] == "yes") {
					...
					if($_manufaktur==""){
						...
						$terusan_manufaktur=" AND pabrik='$_manufaktur' ";
					}
			
					
					$sql="SELECT do_produk.*,pabrik.nama,DATEDIFF('" .date("Y-m-d") ."',tanggal) as usia FROM do_produk,do_produk_detail,pabrik WHERE  do_produk.no_do NOT LIKE '%smt%' and do_produk.no_do NOT LIKE '%btl%'  and  do_produk.no_fin LIKE '%$no_fin' and do_produk.no_do=do_produk_detail.no_do $sql_tbhn and pabrik.id=substring(do_produk.no_do,1,5) and (do_produk.no_do like '$_POST[master]%' or do_produk.tanggal like '$_POST[master]%' or pabrik.nama like '%$_POST[master]%' ) and no_fin like '$no_fin%'  $terusan_manufaktur $addcode $terapp group by do_produk.no_do   ORDER BY approve,tanggal DESC,no_do DESC";
				
					} else {
						$sql="SELECT do_produk.*,pabrik.nama,DATEDIFF('" .date("Y-m-d") ."',tanggal) as usia FROM do_produk,pabrik WHERE no_do NOT LIKE '%smt%' and no_do NOT LIKE '%btl%' and no_do 		NOT LIKE '%btl%' and  no_fin LIKE '%$no_fin'   and (no_do like '$_POST[master]%' or tanggal like '$_POST[master]%' or pabrik.nama like '%$_POST[master]%' )   $sql_tbhn $addcode $terapp  group by do_produk.no_do ORDER BY approve,tanggal DESC,no_do DESC ";
						/*echo "Input Kosong !!!";
	...
	$sql=$sql." limit ".($page*$jmlHal).",".$jmlHal;
		...
		?>
			<tr onMouseOver="this.bgColor = '#CCCC00'" onMouseOut ="this.bgColor = '<?php echo $bgcolor;?>'" bgcolor="<?php echo $bgcolor; ?>">
				<td align="center" width="20" height="43"><?php echo $no; ?></td>
				<td align="left" width="220" height="43">&nbsp;<?php echo $no_do; ?></td>
				<!-- td align="center" width="110" height="20">&nbsp;<?php // echo $style; ?></td -->
				<td align="left" width="130" height="43">&nbsp;<?php echo $tanggal; ?></td>
                <td align="left" width="120" height="43">&nbsp;<?php echo $pabrik; ?></td>
				<td align="center" width="20" height="43">&nbsp;<?php echo $keterangan; ?></td>
				<td align="center" width="20"><?php echo $kode_gudang; ?></td>
				<td align="center" width="20"><?php echo $gudang; ?></td>
				<td align="right" width="90" height="43"><?php echo number_format($totalqty,2,".",",");$totqty=$totqty+$totalqty; ?></td>
				<td align="right" width="100" height="43"><?php echo number_format($hpp,"2",".",",");$tothpp=$tothpp+$hpp;?></td>
				<td align="right" width="100" height="43"><?php echo number_format($totalrp,2,".",",");$totrp=$totrp+$totalrp; ?></td>
		...
		<tr bgcolor="#99CC00">
			...
			<td align="right"><strong><?php echo number_format($totqty,"2",".",",");?></strong></td>
			<td align="right"><strong><?php echo number_format($tothpp,"2",".",",");?></strong></td>
			<td align="right"><strong><?php echo number_format($totrp,"2",".",",");?></strong></td>
			<td colspan="5">&nbsp;</td>
		</tr>
</table>
<script>
        ...
        <tr>
            <td class="text_standard">
                ...
                <?php for($i=0;$i<($jmlData/$jmlHal);$i++){ 
					if($hal<=0){ ?>
						<span class="<?php if($i==$hal) echo "hal_select"; else echo "hal"; ?>" onclick="pindah12('<?php echo $i?>')"><?php echo ($i+1); ?></span>
						<?php if($i>=4) break;
