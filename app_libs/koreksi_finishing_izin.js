	...
	if(ans==true){
	   try{
		  // alert(elm.id + "Value " + value);
		   $.ajax({
			url:"koreksi_finishing_data.php",
			type:"POST",
			cache: false,
			dataType:'text',
 ...
 // alert(nilai);
  ...
  try{
				...
				success: function(data) {	
					...
					if(rsl[0]=='ok'){
					 // alert(rsl[1]);
					   ...
					   alert('Izin Korekai Berhasil Di Proses');
					   window.location="koreksi_finishing_izin.php";		   
					}else{
   ...
   try{
       no++;
      var code='';
	  var dtpTgl='<input style="font-size: 8pt;color:red;height=45;" type="text" name="tglp_'+ no +'" id="tglp_'+ no +'" value="'+ today +'" size="10" class="tanggal"/>';
	  var txtKeterangan='<input style="font-size: 8pt;color:red;height=45;" name="kt_'+ no +'" id="kt_'+ no +'" size="90">';
	  var txtGudang='<input style="font-size: 8pt;color:red;height=45;" name="gd_'+ no +'" id="gd_'+ no +'" size="30" class="gudang">';
	  var lstPilihan='<select style="font-size: 8pt;color:red;height=45;" name="pilihan" id="p_'+ no +'" class="pil"></select>';
	  var btnApprove='<input style="font-size: 8pt;color:red;height=45;" type="button" value="Approved" onclick="update(\''+ no +'\')" id="btnUpdate_'+ no +'"/>';
	 //alert('Try To Add');
	 
	 //asal tbody:first).append jadi tr:first).after
	  jquery4("#myTable tr:first").after("<tr align='center' class='header_table_q' id='data_"+no+"'><td height='45'>"+ no +"</td><td height='45'>"+ dtpTgl +"</td><td height='45'>"+ txtGudang +"</td><td height='45'>"+ lstPilihan +"</td><td height='45' colspan='2'>"+txtKeterangan +"</td><td id='app_"+ no +"'>"+btnApprove +"</td><td id='kb_" + no +"'>&nbsp;</td></tr>");
						
	    jquery4("#tglp_"+ no ).datepicker({
	      dateFormat      : "yy-mm-dd"
	    });
		
		$(".gudang").autocomplete("proses_complete_pabrik.php",{width: 350});
