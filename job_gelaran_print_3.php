...
<?php
	$no_po=$_GET["no_po"];
	$no_co=$_GET["no_co"];
	$sql="select pabrik.id,pabrik.nama,job_gelaran.tujuan_pabrik from pabrik,job_gelaran where pabrik.id=job_gelaran.pabrik and job_gelaran.no_co='$no_co' and job_gelaran.no_po='$no_po'";
	
	echo tujuan_gudang;
	$query=mysql_query($sql)or die($sql);
	list($id_pabrik,$pabrik,$tujuan_gudang)=mysql_fetch_array($query);
...
<td>
	<table>
      ...
      <tr>
        ...
        <td><span class="style3">:</span></td>
        <td class="c_bahan" ><?php echo $c_produk?>&nbsp;&nbsp;&nbsp;</td>
      </tr>
      <tr>
        ...
        <td><span class="style3">:</span></td>
        <td class="c_variasi"><?php echo $c_variasi?> &nbsp;&nbsp;&nbsp;</td>
      </tr>
	   <tr>
      ...
      <tr>
        ...
        <td valign="top"><sub><?php $catatan=str_replace(".","<br>",$catatan); echo $catatan;?></sub></td>
		
      </tr>
    </table>
	<table border="1" width="1300" class="border_tabel" bgcolor="#000000" cellspacing="1" cellpadding="2" bordercolor="#000000">
      <tr bordercolordark="#000000">
        ...
        <th colspan="2"><b><font color="#FFFFFF" face="Verdana, Arial, Helvetica, sans-serif" size="2">Bahan Baku<br />(Realisasi Kg/Roll)</font></b></th>
...
?>
	...
	echo $arrwarna[$kode_war]	?></td>
	    ...
	    <?php }?>
        <?php if($j==1){?>
        <td rowspan="<?php echo $arrcount[$kode_war]?>" align="center"  valign="top" id="supp_<?php echo $kode_war?>" >
		<?php $sql="SELECT
    `j`.`nama`
    , `s`.`nama`
    , `jp`.`qty`
...
<table border="1">
	    ...
	    <?php }?>
        <?php if($j==1){?>
        <td rowspan="<?php echo $arrcount[$kode_war]?>" align="center" id="svariasi_<?php echo $kode_war?>" valign="top" ><?php $sql="SELECT
    `j`.`nama`
    , `s`.`nama`
    , `jp`.`qty`
    , `jp`.`roll`
...
<table border="1">
	...
	if($j==$arrcount[$kode_war]){
      ...
      <tr bgcolor="#666666">
        ...
        <td align="center"><?php echo $kg_bahan_real?></td>
        ...
        <td align="center"><?php echo $kg_variasi_real?></td>
...
?>
	...
	<?php echo"<sub><font color='#FF0000'>-</font></sub>";$catatan_kaki=str_replace(".","<br>-",$catatan_kaki);echo "<sub><font color='#FF0000'>".$catatan_kaki."</font></sub>";?><br />
	<table border="0">
	<tr>
		<td>Mengetahui,</td>
		<td width="300">&nbsp;</td>
