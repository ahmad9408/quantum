...
<?php $content_title = "Control Stok Opname Finishing Suho Garmindo "; ?>
<?php $lihat = 1;
error_reporting(1);
/*
...
$arrayPilihan = array('all' => '--All--', 'formSuho' => 'SUHO', 'formFob' => 'FOB', 'formCmt' => 'CMT');
...
<script language="javascript" src="app_libs/laporan_so_suho_rekap.js?d=<?php echo date('YmdHis') ?>"></script>
...
<form method="post" action="<?php echo $_SERVER["PHP_SELF"]; ?>?action=search" name="f_rekap">
  <table width="600">
    <tr>
      ...
      <td width="9">:</td>
      <td width="391"><select class="form-control" style="width: 100px" name="tipe" id="tipe">
          <?php
    ...
    <tr>
      ...
      <td>:</td>
      <td><input type="text" class="form-control" style="width: 150px" name="tgl1" id="tgl1" value="<?php echo $tgl1 ?>" size="10">
        <script>
          new tcal({
            'controlname': 'tgl1'
          });
    ...
    <tr>
      ...
      <td>:</td>
      <td><input type="text" class="form-control" style="width: 250px" name="barcode" id="barcode" value="<?php echo $barcode ?>" /></td>
    </tr>
    <tr>
      <td><strong>Nama</strong></td>
      <td>:</td>
      <td><input type="text" class="form-control" style="width: 250px" name="txt_nama" id="txt_nama" value="<?php echo $txt_nama; ?>" /></td>
    </tr>
    <tr>
      <td><strong>Jenis Selisih</strong></td>
      <td>:</td>
      <td><select class="form-control" style="width: 100px" name="jenis_selisih" id="jenis_selisih">
          <?php
...
if ($tipe == 'formSuho') {
  $print = '#ReportTable';
} else if ($tipe == 'formFob') {
  $print = '#ReportTableMarkas';
} else if ($tipe == 'formCmt') {
  $print = '#ReportTableDistribusi';
}



?>
<form name="export" method="post" action="produk_list_export.php" onsubmit='$("#datatodisplay").val( $("<div>").append( $("<?php echo $print ?>").eq(0).clone() ).html() )'><input name="Submit" type="submit" value="Export Data">
...
<div id="formSuho">
  <h2>Rekap stock Opname SUHO</h2>
  <span id="procReshare"></span>
  <span id="procReshareKoreksi"></span>
<!--   <span onclick="reFreshReshare()">Refresh Suho</span> -->
  ...
  <table border="1" width="100%" style="font-size: 8pt; width:auto;" cellpadding="0" cellspacing="0" id="ReportTable" class="sortable">
    <tr style="height:20">
      ...
      <td rowspan="2" class="head">&nbsp;Kode Pabrik Suho</td>
      <td rowspan="2" class="head">&nbsp;Nama Pabrik Suho</td>
	...
	*/
    ...
    $sql = "SELECT SQL_CACHE trim(id),trim(nama) from pabrik where id $_pabrik AND status='1' AND id_group='4'";
    ...
    while (list(
      $kode_outlet, $nama, $qty_awal, $hpp_awal, $hpj_awal, $qty_so, $hpp_so, $hpj_so,
      $qty_koreksi, $hpp_koreksi, $hpj_koreksi, $update_date
    ) = mysql_fetch_array($res)) {
      $hpp_awal = number_format($hpp_awal, 2, '.', ',');
...
<div id="formFob">
  ...
  <table border="1" width="100%" style="font-size: 8pt; width:auto;" cellpadding="0" cellspacing="0" id="ReportTableMarkas">
    ...
    $query = "SELECT SQL_CACHE trim(id),trim(nama) from pabrik where id $_pabrik AND status='1' AND id_group='2'";
...
<div id="formCmt">
  <h2>Rekap stock Opname CMT<h2>
      <span id="procDistribusi"></span>
      <span id="procDistribusiKoreksi"></span>
     <!--  <span onclick="reFreshDistribusi()">Refresh CMT</span> -->
      <table border="1" width="100%" style="font-size: 8pt; width:auto;" cellpadding="0" cellspacing="0" id="ReportTableDistribusi">
        <tr style="height:20">
          <td rowspan="2" class="head">No</td>
          <td rowspan="2" class="head">&nbsp;Kode CMT</td>
          <td rowspan="2" class="head">&nbsp;Nama CMT</td>
          <td colspan="3" class="head_dt">Data Stock Awal per <?php echo $tgl1; ?></td>
          <td colspan="3" class="head_dt">&nbsp;Data Stock Awal Yang Telah Di SO</td>
          <td colspan="3" class="head_dt">&nbsp;Stock Opname</td>
          <td colspan="3" class="head_dt">&nbsp;Selisih Plus/minus</td>
        ...
        $sql = "SELECT SQL_CACHE trim(id),trim(nama) from pabrik where id $_pabrik AND status='1' AND id_group='3'";
        //echo $sql;
        // untuk pagging hitung jumlah data di tabel t_postingan
        $qpage = "SELECT FOUND_ROWS() as jumData;";
        $res = mysql_query($sql) or die("wrong syntax query");
...
<script>
  ...
  try {
    ...
    $showTable = '';
    if ($tipe == 'formSuho') {
      $script .= 'getDataAwalSuho();getDataSoSuho();getRekapDataAwalSuho();';
      $showTable .= "$('#formSuho').show();";
    } else if ($tipe == 'formFob') {
      $script .= 'getDataAwalFob();getDataSoFob();getRekapDataAwalFob();';
      $showTable .= "$('#formFob').show();";
    } else if ($tipe == 'formCmt') {
      $script .= 'getDataAwalCmt();getDataSoCmt();getRekapDataAwalCmt();';
      $showTable .= "$('#formCmt').show();";
    } else if ($tipe == 'all'){
      $script .= 'getDataAwalSuho();getDataSoSuho();getRekapDataAwalSuho();';
      $showTable .= "$('#formSuho').show();";
      $script .= 'getDataAwalFob();getDataSoFob();getRekapDataAwalFob();';
    ...
    //alert(e.message);	
    $('#formSuho').text(e.message);
    $('#formFob').text(e.message);
    $('#formCmt').text(e.message);
  }
</script>
