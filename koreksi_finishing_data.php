<?php
  ...
  if($jenis=='ip'){//insert Progress
	 ...
	 $sql="SELECT id,nama FROM pabrik WHERE  id='$kd_gudang'";
	 $res=mysql_query($sql) or die('error#'.$sql.' # '.mysql_error());
	 list($ada)=mysql_fetch_array($res);
	 if(empty($ada)){
	    die('error#Data Pabrik tidak ditemukan dimaster, isi data dengan lengkap');
	 }
  ...
  //    $sql.="VALUES ('$kd','$tgl1','$peminjam','".$kd_barang[0]."','$username');";
	 ...
	 $sql="REPLACE INTO `koreksi_finishing_stok_izin` (id,`id_pabrik`, `tanggal`, `keterangan`,`update_by`,`update_date`,tipe_so)";
	...
	//  $res=mysql_query($sql) or die('error#'.$sql.' # '.mysql_error());
	 ...
	 die("ok#$id#$kd#$kd_barang[0]");
	  ...
	  $sql="update koreksi_finishing_stok_izin set id='BTL.$kd' where id='$kd'";//hapus izin batal lalu
	  mysql_query($sql);
