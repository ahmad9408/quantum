							...
							<table width="100%" border="1">
								...
								<?php
									...
									while(list($koderm,$id_supplier,$satuan,$qtyrm,$kainmasuk)=mysql_fetch_array($hslbrg)){
										...
										$sql="SELECT nama FROM supplier WHERE id='$id_supplier'";
										$hsltemp=mysql_query($sql,$db);
		...
		<table width="100%">
			<tr>
				<td align="center">
					...
					<?php
						if($approved){
							?><input type="button" value="Print Mode" onclick="window.open('job_gelaran_print.php?no_po=<?php echo $no_po; ?>&no_co=<?php echo $no_co; ?>','job_gelaran_print','width=800,height=400,menubar=yes,scrollbars=yes');"><?php
						}
					?>
