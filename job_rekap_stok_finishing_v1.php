...
if (@$_POST['submit'] == 'export' || $detail == 1 || @isset($_POST['btnExportDataStok'])) {
	...
	if ($csv == 0) {
		...
		if ($isExportStokOnly == 1) {
			header("Content-Disposition: attachment;filename=" . $pabrik1 . $txtprodukpilihan . "StokOnly" . $time . "." . $ext);
		} else {
			header("Content-Disposition: attachment;filename=" . $pabrik1 . $txtprodukpilihan . "_" . $time . "." . $ext);
		}
		...
		if ($isExportStokOnly == 1) {
			...
			$myFile = 'StokOnly' . $txtprodukpilihan . str_ireplace('.', '', $txtmarkas . $pabrik1) . '_' . $txtjenis . '_' . $filter_periode .
				'_' . $hari_download . '_' . $tanggal . '_' . date('His') . '.' . $ext;
		} else {
			#$filename=$pabrik1."_".$time.".".$ext;
			$myFile = $txtprodukpilihan . str_ireplace('.', '', $txtmarkas . $pabrik1) . '_' . $txtjenis . '_' . $filter_periode . '_' . $hari_download . '.' . $ext;
		}
	...
	// $sql_inner_group = ' INNER JOIN outlet o on o.id=r.kode_manufaktur ';
	...
	if (!empty($txtprodukpilihan)) {
		...
		$sql = "SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),r.kode_manufaktur,r.nama_manufaktur,
				...
				FROM $table_rekap_stok as r WHERE  $filter and r.periode like '$filter_periode%' $filter_barcode $sql_tambahan $sql_area ORDER by  r.kode_manufaktur,r.barcode_15; -- sql 78 ";
		#die($sql); 
		...
		if ($isExportStokOnly == 1) { //dipindah ke rekap_stok_outlet_complete_stock_only_export.php
			...
			$sql .= "FROM $table_rekap_stok  r  WHERE  r.periode like '$filter_periode%' ";
			$sql .= " $filter_barcode $sql_area group by r.kode_manufaktur,r.barcode_15;  -- sql 9";
		} else {
			$sql = "SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),r.kode_manufaktur,r.nama_manufaktur,";
			...
			$sql .= "FROM $table_rekap_stok r WHERE  r.periode like '$filter_periode%' ";
			$sql .= " $filter_barcode $sql_area  group by r.kode_manufaktur,r.barcode_15; -- sql 87";
		}
	} else {
		if ($isExportStokOnly == 1) {
			...
			$sql = "SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),r.kode_manufaktur,r.nama_manufaktur,
				...
				FROM $table_rekap_stok r WHERE r.kode_manufaktur='$pabrik1'  and r.periode like '$filter_periode%' $filter_barcode $sql_area ; -- sql10";
		} else {
			$sql = "SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(tgl_stok_awal,'%d %M %Y'),r.kode_manufaktur,r.nama_manufaktur,
				...
				FROM $table_rekap_stok r WHERE r.kode_manufaktur='$pabrik1' and r.periode like '$filter_periode%' $filter_barcode $sql_area ; ";
		}
	}
...
<?php if ($export_to_excel <> '1') { ?>
	...
	<fieldset>
		<form method="POST" id="frmFilter" name='outlet' action="<?php echo $_SERVER["PHP_SELF"]; ?>?action=search" onsubmit="return validation();">
			...
			<table class="table table-bordered" width="1654" style="font-size: 10pt" height="28">
				<tr>
				...
				<tr>
					<td><b>Jenis Pabrik</b></td>
					<td>:</td>
					<td>
						<select size="1" name="txtgroup" style="font-size: 8pt;width:100px;" id="txtgroup">
							...
							echo "<option value='1' $sel1>SUHO</option>";
							echo "<option value='2' $sel2>FOB</option>";
							echo "<option value='2' $sel2>CMT</option>";
							?>
						</select>
					</td>
					<td>&nbsp;</td>
	...
	if ($search == "search") {
		...
		if (!empty($txtprodukpilihan)) {
			...
			if (isset($cari_barcode)) {
				...
				$sql .= "	FROM $table_rekap_stok as r 
				...
				$sql .= "	 $filter_minus $tambahanPencarianBerdasarkanBarcode	  $sql_area  $sql_aktif  $sql_tbhn2 order by kode_manufaktur limit $jumlah_data" . '; -- akses 41';
			} else {
				$sql = "SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),upper(r.kode_manufaktur),r.nama_manufaktur, ";
			...
			$sql .= "r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
			$sql .= "r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql .= "FROM $table_rekap_stok r WHERE kode_manufaktur='$pabrik1'  and periode like '$filter_periode%' $sql_area  $sql_tbhn2 ";
			...
			$sql .= "r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
			$sql .= "r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql .= "	FROM $table_rekap_stok r WHERE r.kode_manufaktur like '$pabrik1%'  $sql_aktif $tambahan and r.periode like '$filter_periode%'  ";
			...
			$sql .= " SUM(r.k21),SUM(r.k22),SUM(r.k23),SUM(r.k24),SUM(r.k25),SUM(r.k26),SUM(r.k27),SUM(r.k28),SUM(r.k29),SUM(r.k30),SUM(r.k31), ";
			$sql .= " sum(r.stok_akhir),sum(r.hpp*r.stok_akhir),sum(r.hpj*r.stok_akhir) ";
			$sql .= "FROM $table_rekap_stok as r WHERE r.kode_manufaktur like '$pabrik1%' $tambahan AND r.periode like '$filter_periode%' $filter_barcode $sql_area $sql_aktif $sql_tbhn2";
		...
		if ($detail == 1) {
			...
			$sql .= "r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
			$sql .= "r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql .= "FROM $table_rekap_stok r WHERE r.kode_manufaktur='$pabrik1' and o.is_show_omset=1 and r.periode like '$filter_periode%' $sql_tbhn2 order by r.kode_manufaktur LIMIT " . ($page * $jmlHal) . "," . $jmlHal . '; -- akses 3';
		} elseif ((isset($cari_barcode) && (!empty($barcode) || !empty($txt_nama))) || isset($cari_minus)) {
			if (isset($cari_minus)) {
				$filter_minus = ' AND stok_akhir<0';
			} else {
			...
			$sql .= "r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
			$sql .= "r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql .= "	FROM $table_rekap_stok r WHERE r.kode_manufaktur like '$pabrik1%' $tambahan and r.periode like '$filter_periode%'  ";
			...
			$sql .= " SUM(r.k21),SUM(r.k22),SUM(r.k23),SUM(r.k24),SUM(r.k25),SUM(r.k26),SUM(r.k27),SUM(r.k28),SUM(r.k29),SUM(r.k30),SUM(r.k31), ";
			$sql .= " sum(r.stok_akhir),sum(r.hpp*r.stok_akhir),sum(r.hpj*r.stok_akhir)  ";
			$sql .= "FROM $table_rekap_stok r WHERE r.kode_manufaktur like '$pabrik1%' $tambahan  and r.periode like '$filter_periode%' $sql_area ";
