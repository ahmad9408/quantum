                    ...
                    data: [],
					 point: {
						events: {
							click: function(e) {
								//this.slice();
								console.log('ini d kilik ' + e.point.name + ' ' + e.point.y) ; 
								//generatePiePerkelasByAge(e.point.name);
								loadDetailAge(e.point.name);
								//location.href = e.point.url; redirece
								//e.preventDefault();//redirec
...
function generatePiePerkelasAge(nama){
	console.log('generatePiePerkelasByAge by kelas' + nama ) ; 	
	optionsCharKelasAge = {
                chart: {
                    renderTo: 'container_detail_age',
                    plotBackgroundColor: null,
                    plotBorderWidth: null,
                    plotShadow: false
                },
                title: {
                    text: 'Penjualan Produk Kelas '+ nama +' Outlet Menurut Usia'
                },
                tooltip: {
                    formatter: function() {
                        return '<b>'+ this.point.name +' Nilai '+ this.point.y +' from ' + this.total 
                    ...
                    data: [],
					 point: {
						events: {
							click: function(e) {
								//this.slice();
								console.log('ini d per Age ' + e.point.name + ' ' + e.point.y) ; 
								//location.href = e.point.url; redirece
								//e.preventDefault();//redirec
								//alert(e.point.name + ' ' + e.point.y);//alert disini
							}
...
function generatePiePerkelasByGender(nama){	
	optionsCharKelas = {
                chart: {
                    renderTo: 'container_detail_gender',
                    plotBackgroundColor: null,
                    plotBorderWidth: null,
                    plotShadow: false
                },
                title: {
                    text: 'Penjualan Produk Kelas '+ nama +' Outlet Menurut gender'
                },
                tooltip: {
                    formatter: function() {
                        return '<b>'+ this.point.name +' Nilai '+ this.point.y +' from ' + this.total 
...
function loadDetailAge(kelasName){
	console.log('loadDetailAge ' + kelasName)
	generatePiePerkelasAge(kelasName);
	$('#container_detail_age').show();
	$('#container_detail_age').html('<div align="center" style="margin-top:100;"><img name="load" src="ajax-loader.gif" width="100" height="100" alt="" /><br><h2>GET DATA ....</h2></div>');
	 var tgl1=$('#tgl1').val();
	 var tgl2=$('#tgl2').val();
	 var tipe=$('#tipe').val();
	 var outlet=$('#outlet').val();
	  ...
	  $.ajax({
			...
			cache: false,
			dataType:'json', 
			data:{j:'gsrka',t1:tgl1,t2:tgl2,o:outlet,t:tipe,k:kelasName,m:markas},
			success: function(data) {
				//$('#container_detail').text(data);return;
				try{
					 optionsCharKelasAge.series[0].data = data;
                     chart = new Highcharts.Chart(optionsCharKelasAge);
				}catch(e){
					alert(e.message);
				}
