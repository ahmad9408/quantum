<?php session_start();?>
<?php $content_title="Rekap Stok Outlet All"; 
...
$area_lain=$_SESSION['area_lain'];
 ...
 $data_tambahan=" r.kode_outlet like '$area%' ";
  if(!empty($area_lain)){
	  $d=explode(';',$area_lain); 
	  $d_lain='';
	  foreach($d as $kd_lain){
		  $kd_lain=trim($kd_lain);
		    if(!empty($kd_lain)){
			   	$d_lain.=" OR r.kode_outlet LIKE '$kd_lain%' ";
			}
	  }
...
#$sql_area= " AND r.kode_outlet like '$area%' ";
$sql_area="  AND ( $data_tambahan ) ";



$sql_tambahan=" AND r.kode_outlet like '%0%' and length(trim(r.kode_outlet))=10 ";// add tgl 22 jan 2014
@$txt_organization=$_POST['txt_organization'];	
$sql_cache=' SQL_CACHE ';

@$txtTable=$_POST['txtTable'];
if(empty($txtTable)){
	$table='rekap_stok_outlet';
...
if(@$_POST['submit']=='export'||$detail==1 ||@isset($_POST['btnExportDataStok'])){
  ...
  if($filter_periode!=date('Y-m')){
	   $table_rekap_stok=$table.'_'.$bulan1.$tahun1;  
	   $hari_download='';
  }else{
	   ...
	   if(@isset($_POST['btnExportDataStok'])){
		...
		if($csv==0){
			...
			if($isExportStokOnly==1){
			  header("Content-Disposition: attachment;filename=".$txtoutlet.$txtprodukpilihan."StokOnly".$time.".".$ext );			   
			}else{
			   header("Content-Disposition: attachment;filename=".$txtoutlet.$txtprodukpilihan."_".$time.".".$ext );
			}
			...
			if($isExportStokOnly==1){
			  #$filename=$txtoutlet."StokOnly".$time.".".$ext;	
			  
			  $myFile = 'StokOnly'.$txtprodukpilihan.str_ireplace('.','',$txtmarkas.$txtoutlet).'_'.$txtjenis.'_'.$filter_periode.
			           '_'.$hari_download.'_'.$tanggal.'_'.date('His').'.'.$ext;	   
			}else{
			  #$filename=$txtoutlet."_".$time.".".$ext;
			  $myFile = $txtprodukpilihan.str_ireplace('.','',$txtmarkas.$txtoutlet).'_'.$txtjenis.'_'.$filter_periode.'_'.$hari_download.'.'.$ext;
			}
			...
			#$myFile = str_ireplace('.','',$txtmarkas.$txtoutlet).'_'.$txtjenis.'_'.$filter_periode.'_'.$hari_download.'.'.$ext;
			...
			@$result_delete=exec("/bin/rm -f $destination".$txtmarkas.$txtoutlet."_".$filter_periode."_*");
			
			#echo "Result Delete $result_delete $myFile "."/bin/rm -f $destination".$txtmarkas.$txtoutlet."_".$filter_periode."_*";
  ...
  if(!empty($outlet_export)){
	  ...
	  $sql_pilih_reshare=" AND r.kode_outlet in ($data_in) ";
  }else{
	 $sql_pilih_reshare='';  
  }
  
  $sql_inner_outlet=' INNER JOIN outlet o on o.id=r.kode_outlet ';
  ...
  $filter_barcode='';
   ...
   if(!empty($txtprodukpilihan)){
	   ...
	   if(trim($txtoutlet)=='ALL'){
		   $filter=" r.jenis like '$txtjenis%' ";   
	   }else{
		   $filter=" r.kode_outlet='$txtoutlet' ";  
	   } 
	   
	   
	   
			$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),r.kode_outlet,r.nama_outlet,
				...
				r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(0),(r.hpj)
				FROM $table_rekap_stok as r inner join produk_pilihan pp on pp.id_barang=r.barcode_15 $sql_inner_outlet WHERE  $filter and r.periode like '$filter_periode%' $filter_barcode and pp.status=1 and pp.pilihan='$txtprodukpilihan' $sql_tambahan $sql_aktif  $sql_pilih_reshare $sql_area ORDER by  r.kode_outlet,r.barcode_15; -- sql 78 ";
				#die($sql); 
		}elseif(trim($txtoutlet)=='ALL'){
		   ...
		   if($isExportStokOnly==1){//dipindah ke rekap_stok_outlet_complete_stock_only_export.php
			...
			$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),r.kode_outlet,r.nama_outlet,";
			$sql.="r.barcode_13,r.barcode_15,r.nama,r.hpj,";
			$sql.="sum(r.stok_akhir),sum(  $field_hpp_export   * r.stok_akhir),sum(r.hpj* r.stok_akhir)";
			$sql.="FROM $table_rekap_stok  r  $sql_inner_outlet WHERE  r.periode like '$filter_periode%' and r.jenis like '$txtjenis%' ";
			$sql.=" $filter_barcode 	$sql_aktif $sql_area $sql_pilih_reshare  group by r.kode_outlet,r.barcode_15;  -- sql 9"; 
		   }else{
			  $sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),r.kode_outlet,r.nama_outlet,";
		   $sql.="r.barcode_13,r.barcode_15,r.nama,$field_hpp_export hpp,r.hpj , sum( $field_hpp_export * r.stok_awal),sum(r.hpj* r.stok_awal),sum(r.stok_awal),";
			...
			$sql.="sum(r.k21),sum(r.k22),sum(r.k23),sum(r.k24),sum(r.k25),sum(r.k26),sum(r.k27),sum(r.k28),sum(r.k29),sum(r.k30),sum(r.k31),sum(r.stok_akhir),sum( $field_hpp_export *r.stok_akhir),sum(r.hpj*r.stok_akhir)";
			$sql.="FROM $table_rekap_stok r  $sql_inner_outlet WHERE  r.periode like '$filter_periode%' and r.jenis like '$txtjenis%' ";
			$sql.=" $filter_barcode  $sql_aktif	 $sql_pilih_reshare $sql_area  group by r.kode_outlet,r.barcode_15; -- sql 87"; 
		   }
	    ...
	    if($isExportStokOnly==1){
			...
			$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),r.kode_outlet,r.nama_outlet,
				r.barcode_13,r.barcode_15,r.nama,r.hpj,$field_stok_akhir as stok_akhir,(  $field_stok_akhir * $field_hpp_export ) as hpp,( r.hpj * $field_stok_akhir) as hpj
				FROM $table_rekap_stok r  $sql_inner_outlet  WHERE r.kode_outlet='$txtoutlet'  and r.periode like '$filter_periode%' $filter_barcode  $sql_aktif $sql_pilih_reshare $sql_area ; -- sql10";
		}else{
			$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(tgl_stok_awal,'%d %M %Y'),r.kode_outlet,r.nama_outlet,
				...
				r.ri21,r.ri22,r.ri23,r.ri24,r.ri25,r.ri26,r.ri27,r.ri28,r.ri29,r.ri30,r.ri31,
				r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20,
				r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,( r.stok_akhir * $field_hpp_export ) as hpp_stok_akhir,( r.stok_akhir * r.hpj) hpj_stok_akhir
				FROM $table_rekap_stok r $sql_inner_outlet WHERE r.kode_outlet='$txtoutlet'  and r.periode like '$filter_periode%' $filter_barcode  $sql_aktif $sql_pilih_reshare $sql_area ; ";
		}
	...
	if(empty($row_found)){
		die("Data tidak ditemukan silahkan cek pilihan datanya");
	}
	...
	if($csv==0){
		...
		xlsWriteLabel($row,4,"Kode Reshare");
		xlsWriteLabel($row,5,"Nama Reshare");
	   ...
	   xlsWriteLabel($row,++$column,'Total In Reload ');
		for($i=1;$i<=31;$i++){
		  $column++;
	      eval("xlsWriteLabel($row,$column,$i);");		  
	   }
	   	xlsWriteLabel($row,++$column,'Total Out Sales ');
		for($i=1;$i<=31;$i++){
		  $column++;
	      eval("xlsWriteLabel($row,$column,$i);");		  
	   }
	   ...
	   xlsWriteLabel($row,++$column,'Stok Akhir HPJ');
		...
		if($isExportStokOnly==1){
			...
			$judul="No $se Periode $se Kode Reshare $se Nama Reshare $se Barcode 13 $se Barcode 15 $se Produk $se Hpj $se";
			...
			$judul.="In Reload => $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se Total In Reload $se " ;
			$judul.="Out Retur=> $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se Total Retur Out $se " ; 
			$judul.="Out Sales => $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se \t $se Total Stok Out $se " ;
			...
			$judul="No $se Periode $se Tanggal Stok Awal $se Kode Reshare $se Nama Reshare $se Barcode 13 $se Barcode 15 $se Produk $se   " ; 
			if($isShowHpp==1){
				$judul.="Hpp $se ";
			}
			$judul.="Hpj $se Qty $se ";
    ...
    if($txtoutlet=='ALL'){
		$sql_outlet="SELECT $sql_cache UPPER(id),nama from outlet where id like '%' and jenis in (1,2) and `type`=4 and is_show_omset=1;";
	}else{
		$sql_outlet="SELECT $sql_cache UPPER(id),nama from outlet where id like '$txtoutlet%';";
	}
    ...
    if($isExportStokOnly==1){
		while ( list($periode,$kode_outlet,$nama_outlet,$barcode_13,$barcode_15,$nama,$hpj,
			$stok_akhir,$stok_akhir_hpp,$stok_akhir_hpj)=mysql_fetch_array($hsltemp)){
				//ambil data 
				$row++;
		        $i++;
			$nama_outlet=$arrayOutlet[strtoupper($kode_outlet)];
			...
			$data="$i $se ' $periode $se $kode_outlet $se $nama_outlet $se $barcode_13 $se $barcode_15 $se $nama $se $hpj $se " ; 	
	 ...
	 while ( list($periode,$tgl_stok_awal,$kode_outlet,$nama_outlet,$barcode_13,$barcode_15,$nama,$hpp,$hpj,$hpp_stok,$hpj_stok,$stok_awal,
			...
			$k21,$k22,$k23,$k24,$k25,$k26,$k27,$k28,$k29,$k30,$k31,$stok_akhir,$stok_akhir_hpp,$stok_akhir_hpj)=mysql_fetch_array($hsltemp)){
		$row++;
		$i++;
		...
		*/
		if($isShowHpp==1){
			...
			$hpp=0;$hpp_stok=0;$stok_akhir_hpp=0;	
		}
		...
		if($csv==0){
			...
			xlsWriteLabel($row,4,$kode_outlet );
			xlsWriteLabel($row,5,$nama_outlet);
			...
			$data="$i $se ' $periode $se ' $tgl_stok_awal $se $kode_outlet $se $nama_outlet $se $barcode_13 $se $barcode_15 $se $nama $se " ; 
			if($isShowHpp==1){
				$data.="$hpp $se ";
			}
...
@$txtmarkas=sanitasi($_POST['txtmarkas']);
echo 'Markas :' .$txtmarkas;
if(isset($txtmarkas)){
	//$area=substr($txtmarkas,0,4);;
}
if(@isset($_POST['txtoutlet'])){
    @$txtoutlet=sanitasi($_POST['txtoutlet']); 
}else{
    if(isset($_SESSION['txtoutlet'])){
        $txtoutlet= $_SESSION['txtoutlet'];
	}else{
		if(!isset($_REQUEST['action'])){
			$txtoutlet='ALL';// awal buka page
			$txtmarkas='ALL';
		}
...
<script src="app_libs/rekap_stok_outlet_complete_v14.js?d=<?php echo date('YmdHis');?>"></script>
<script language="JavaScript">
	$(document).ready(function(){
	  ...
	  <?php
		...
		//$('.stok').hide();	
		//$('#txtjenis').hide();
		$('#txtoutlet').change(function(){
		   if($('#txtoutlet').val()=='ALL'){
			   $('#txtjenis').show();
		   }else{
			   $('#txtjenis').hide();
		   }
...
<table border="0" width="1654" style="font-size: 10pt" height="28">
    <tr>
      <td><?php echo $data_global['f_organization']; ?></td>
      <td>:</td>
      <td><b><font color="#0000FF">
        <select size="1" name="txt_organization" style="font-size: 8pt;width:200px;" id="txt_organization">
        ...
        <td width="633">&nbsp;
          <table width="400" border="1">
            <tr>
              ...
              <td width="8">:</td>
              <td width="295"><input name="barcode" type="text" id="barcode" value="<?php echo $barcode?>" 
              size="70" maxlength="200"/></td>
            </tr>
            <tr>
              <td>Nama</td>
              <td>:</td>
              <td><input name="txt_nama" type="text" id="txt_nama" value="<?php echo $txt_nama?>" size="40" maxlength="50"/></td>
            </tr>
          </table>
        <td width="272">
      <strong><font color="#FF0000">Jumlah Data max 9999</font></strong>
      <input name="jumlah_data" type="text" id="jumlah_data" value="<?php echo $jumlah_data?>" size="4" maxlength="4"/></tr>
    <tr>
      <td><b><?php echo $data_global['outlet_name']; ?></b></td>
      <td>:</td>
      <td><b><font color="#0000FF">
        <select size="1" name="txtoutlet" style="font-size: 8pt;width:300px;" id="txtoutlet"  >
          <option value="--">--Select--</option>
          <?php
		   //$sql="SELECT SUBSTR(outlet,1,4) FROM user_account WHERE username ='$username'";
		   //$res=mysql_query($sql) or die($sql .'  # '.mysql_error());		   
		   //list($markas)=mysql_fetch_array($res);
		   if($username=="superuser"||$username=="budi-it"||$username=="r_simulasi"){
			   $sql_otl="select id, nama from outlet where  (type=4  and jenis in (1,2) and is_show_omset=1 ) or ( id like 'MSB1%')  order by id"; 
		   }else{
			   $data_tambahan=" id like '$area%' ";
			  if(!empty($area_lain)){
				  $d=explode(';',$area_lain); 
				  $d_lain='';
				  foreach($d as $kd_lain){
					  $kd_lain=trim($kd_lain);
						if(!empty($kd_lain)){
							$d_lain.=" OR id LIKE '$kd_lain%' ";
						}
				  }
				   $data_tambahan.=$d_lain;
			  } 
			  
			  $sql_tbhn=" AND ( $data_tambahan ) ";
			   
			   $sql_otl="select id, nama from outlet where  type=4  and jenis in (1,2) and is_show_omset=1 $sql_tbhn order by id";  
		   }
		   ...
		   $result=mysql_query($sql_otl) or die($sql_otl);
		   $arrayReshare=array();
		   while(list($kd_otl,$nm_otl)=mysql_fetch_array($result)){
			      $arrayReshare[$kd_otl]=$nm_otl;
		          if($txtoutlet==$kd_otl){
				     echo '<option value="'.$kd_otl.'" selected>'."[$kd_otl] $nm_otl".'</option>';
				  }else{
				     echo '<option value="'.$kd_otl.'">'."[$kd_otl] $nm_otl".'</option>';
				  }
      ...
      <td>&nbsp;</td>
      <td><b><font color="#0000FF">
        <select size="1" name="txtjenis" style="font-size: 8pt;width:100px;" id="txtjenis">
          <option value="">--Select--</option>
          <?php
			       if($txtjenis=='1'){
					   $sel1=' selected ';
		           ...
		           echo "<option value='1' $sel1>Internal</option>";
		           echo "<option value='2' $sel2>Mitra</option>";
		   
		
		
		?>
          </select>
        <input type="submit" name="CariBarcode" id="CariBarcode" value="Lihat Brdasarkan Barcode" style="font-size: 8pt"/>
        <input type="submit" name="CariMinus" id="CariMinus" value="Lihat Data Minus" style="font-size: 8pt"/>
      </font></b></td>    
      <td>Style :
      <input name="txt_model" type="text" id="txt_model" value="<?php echo $nama_model?>"  size="30" onclick="clearBarcode();"/></tr>
    <tr>
      <td><strong>Markas</strong></td>
      <td>:</td>
      <td><b><font color="#0000FF">
        <select size="1" name="txtmarkas" style="font-size: 8pt;width:300px;" id="txtmarkas">
          <option value="">--Select--</option>
          <?php
		   //$sql="SELECT SUBSTR(outlet,1,4) FROM user_account WHERE username ='$username'";
		   //$res=mysql_query($sql) or die($sql .'  # '.mysql_error());		   
		   ...
		   #$sql_otl="select substr(id,1,4) as id, nama from outlet where  type=1  $sql_tbhn order by id";
		   $sql_otl="select id as id, nama from outlet where  type=1  $sql_tbhn order by id";
		   $result=mysql_query($sql_otl) or die($sql_otl);
		   while(list($kd_markas,$nm_otl)=mysql_fetch_array($result)){
		          if($kd_markas==$txtmarkas){
				     echo '<option value="'.$kd_markas.'" selected>'."[$kd_markas] $nm_otl".'</option>';
				  }else{
				     echo '<option value="'.$kd_markas.'">'."[$kd_markas] $nm_otl".'</option>';
				  }
		          
		   }
		   
		    if(trim($txtmarkas)=='ALL'){
		       echo '<option value="ALL" selected>--ALL--</option>';
		   }else{
		       echo '<option value="ALL">--ALL--</option>';
		   }
      ...
      <td ondblclick="showTable();"> <input name="txt_supp" type="text" id="txt_supp" value="<?php echo $txt_supp?>" size="40" maxlength="50"/><br />
        Kode Supplier :
        <input name="txt_kode_supp" type="text" id="txt_kode_supp" value="<?php echo $txt_kode_supp; ?>" size="3" maxlength="3"/></td>
      <td>&nbsp;</td>   
    </tr>
    <tr id="listProdukPilihan">
      <td>Produk Pilihan</td>
      <td>&nbsp;</td>
      <td><select size="1" name="txtprodukpilihan" id ="txtprodukpilihan" style="font-size: 8pt;width:300px;">
        <option value=""></option>
        <?php
				   $sql="SELECT pilihan FROM produk_pilihan_mst WHERE aktif=1 order by pilihan";
				   $res=mysql_query($sql);
			      while(list($pilihan)=mysql_fetch_array($res)){
					  if($pilihan==$txtprodukpilihan){
						  echo "<option value='$pilihan' selected='selected'>$pilihan</option>";
					  }else{
    ...
    <tr>
      ...
      <td colspan="4"><b>
        ...
        <a href="#" id="show">Show</a>&nbsp;&nbsp;<a href="#" id="hide">hide</a>&nbsp;&nbsp;<a href="#" id="show_stok">.</a>&nbsp;&nbsp;<a href="#" id="hide_stok">,</a><font color="#0000FF">
          <input type="submit" value="lihat" name="btnLihat" style="font-size: 8pt" id="btnLihat" />
          <input type="submit" value="export" name="submit" style="font-size: 8pt" />
          </font></b>&nbsp;<input name="txtTable" type="text" id="txtTable" value="<?php echo $table;?>" style="display:none;"/>
        <b>
      ...
      <td><input type="button" name="cariStyle" id="cariStyle" value="Cari Model" onclick="cariProduk();"  style="font-size: 8pt"/>      </tr>
    <tr>
      <td>&nbsp;</td>
      <td colspan="4"><b>
      
        <input type="button" value="Export hanya Stok Akhir" name="btnExportDataStok3" style="font-size: 8pt" id="btnExportDataStok3" onclick="exportStokAkhir()" />
        <span id="id_tanggal" <?php if($username=='budi-it'){}else{ /*echo 'style="display:none;"';*/}?>> Tanggal   </span>
...
<h4>Kode Reshare Yang akan diexport</h4>
...
<h3>Data Reshare yang akan dieksport</h3>
...
<table border="0" width="100%" style="font-size: 8pt" height="126" class="sortable">
  <thead>
  <tr> 
    ...
    <td colspan="<?php echo ($jpb+1)?>" align="center" <?php echo $bgc_si?> class="header">IN RELOAD</td>
    <td colspan="<?php echo ($jpb+1)?>" align="center" <?php echo $bgc_ro?> class="header">OUT RETUR</td>
    <td colspan="<?php echo ($jpb+1)?>" align="center" <?php echo $bgc_so?> class="header">OUT SALES</td>
  ...
  <?php
	...
	echo "<h2>Cari Barcode Value $cari_barcode</h2>";
    if ($search=="search"){
        session_start();
        ...
        if(trim($txtoutlet)=='ALL'){
		     $tambahan=" And r.jenis like '$txtjenis%' and r.kode_outlet not like '%O0000' and r.kode_outlet like '$area%'";
			 $txtoutlet='';
		}
        
        //$sql1="SELECT count(tanggal) FROM $table  where kode_outlet like '$txtoutlet%' and tanggal between '$tgl1' and '$tgl2' $tambahan";
		$sql_inner_produk='';
		$sql_tbhn2='';
		...
		if(!empty($txt_organization)){
			$sql_tbhn2.=" AND r.kode_outlet like '$txt_organization%' ";
		}
		...
		if(!empty($txtprodukpilihan)){
			...
			if(isset($cari_barcode)){
				$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),upper(r.kode_outlet),r.nama_outlet, ";
				...
				$sql.="	r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp* r.stok_akhir),(r.hpj* r.stok_akhir) ";
				$sql.="	FROM $table_rekap_stok as r  $sql_inner_outlet INNER JOIN produk_pilihan pp on pp.id_barang=r.barcode_15 
				
				 $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%'  $tambahan and r.periode like '$filter_periode%'  AND pp.status=1 and pp.pilihan='$txtprodukpilihan'";
				$sql.="	 $filter_minus $tambahanPencarianBerdasarkanBarcode	  $sql_area  $sql_aktif  $sql_tbhn2 order by kode_outlet limit $jumlah_data".'; -- akses 41';
			}else{
				$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(r.tgl_stok_awal,'%d %M %Y'),upper(r.kode_outlet),r.nama_outlet, ";
				...
				$sql.=" sum(r.stok_akhir),sum(r.hpp*r.stok_akhir),sum(r.hpj*r.stok_akhir) ";
				$sql.="	FROM $table_rekap_stok as r  $sql_inner_outlet INNER JOIN produk_pilihan pp on pp.id_barang=r.barcode_15 
				
				 $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%'  $tambahan and r.periode like '$filter_periode%'  AND pp.status=1 and pp.pilihan='$txtprodukpilihan'";
				$sql.="	 $filter_minus $tambahanPencarianBerdasarkanBarcode	  $sql_area  $sql_aktif  $sql_tbhn2 
				group by DATE_FORMAT(r.periode,'%M-%Y'),r.kode_outlet  order by kode_outlet limit $jumlah_data".'; -- akses 41group';
			}
				...
				$sql.="r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
				$sql.="r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
				$sql.="FROM $table_rekap_stok r $sql_inner_produk WHERE kode_outlet='$txtoutlet'  and periode like '$filter_periode%' $sql_area  $sql_tbhn2 ";
				$sql.="order by kode_outlet LIMIT ".($page*$jmlHal).",".$jmlHal;
		}elseif((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			if(isset($cari_minus)){$filter_minus=' AND stok_akhir<0';}else{$filter_minus='';}
			if(!empty($barcode)){
				#$tambahanPencarianBerdasarkanBarcode=" and ( barcode_15 like '$barcode%') ";
				$dt=explode(',',$barcode);
				$lstbr='';
			...
			$sql.="r.si1,r.si2,r.si3,r.si4,r.si5,r.si6,r.si7,r.si8,r.si9,r.si10,r.si11,r.si12,r.si13,r.si14,r.si15,r.si16,r.si17,r.si18,r.si19,r.si20, ";
				...
				$sql.="r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
				$sql.="r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql.="	FROM $table_rekap_stok r inner join outlet o on o.id=r.kode_outlet $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%'  $sql_aktif $tambahan and r.periode like '$filter_periode%'  ";
			$sql.="	 $filter_minus $tambahanPencarianBerdasarkanBarcode	 $sql_area $sql_tbhn2 order by r.kode_outlet limit $jumlah_data".'; -- akses 3';
			...
			if(!empty($barcode)){
				...
				$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(tgl_stok_awal,'%d %M %Y'),upper(r.kode_outlet),r.nama_outlet, ";
				...
				$sql.=" SUM(r.k21),SUM(r.k22),SUM(r.k23),SUM(r.k24),SUM(r.k25),SUM(r.k26),SUM(r.k27),SUM(r.k28),SUM(r.k29),SUM(r.k30),SUM(r.k31), ";
				$sql.=" sum(r.stok_akhir),sum(r.hpp*r.stok_akhir),sum(r.hpj*r.stok_akhir) ";
				$sql.="FROM $table_rekap_stok r $sql_inner_outlet  $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%' $tambahan  and r.periode like '$filter_periode%'  $filter_barcode $sql_area  $sql_aktif  $sql_tbhn2 ";
				$sql.="GROUP BY r.kode_outlet,DATE_FORMAT(r.periode,'%Y-%m') order by r.kode_outlet LIMIT ".($page*$jmlHal).",".$jmlHal.'; -- akses 42';	
		}
        
    } else {
        
        //$sql1="SELECT count(tanggal) FROM $table  where kode_outlet like '$txtoutlet%' and tanggal between '$tgl1' and '$tgl2' And jenis=1";
        if($detail==1){
				$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(tgl_stok_awal,'%d %M %Y'),upper(r.kode_outlet),r.nama_outlet, ";
				$sql.="r.barcode_13,r.barcode_15,r.nama,(r.hpp*r.stok_awal),(r.hpj*r.stok_awal),r.stok_awal, ";
				$sql.="r.si1,r.si2,r.si3,r.si4,r.si5,r.si6,r.si7,r.si8,r.si9,r.si10,r.si11,r.si12,r.si13,r.si14,r.si15,r.si16,r.si17,r.si18,r.si19,r.si20, ";
				...
				$sql.="r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
				$sql.="r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
				$sql.="FROM $table_rekap_stok r  inner join outlet o on o.id=r.kode_outlet  $sql_inner_produk  WHERE r.kode_outlet='$txtoutlet' and o.is_show_omset=1 and r.periode like '$filter_periode%' $sql_tbhn2 order by r.kode_outlet LIMIT ".($page*$jmlHal).",".$jmlHal.'; -- akses 3';
		}elseif((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			if(isset($cari_minus)){$filter_minus=' AND stok_akhir<0';}else{$filter_minus='';}
			$sql="SELECT $sql_cache DATE_FORMAT(r.periode,'%M-%Y'),DATE_FORMAT(tgl_stok_awal,'%d %M %Y'),upper(r.kode_outlet),r.nama_outlet, ";
			$sql.="	IF(LENGTH(r.barcode_13)=0,r.barcode_15,r.barcode_13),r.barcode_15,r.nama,(r.hpp*r.stok_awal),(r.hpj*r.stok_awal),r.stok_awal, ";
			$sql.="r.si1,r.si2,r.si3,r.si4,r.si5,r.si6,r.si7,r.si8,r.si9,r.si10,r.si11,r.si12,r.si13,r.si14,r.si15,r.si16,r.si17,r.si18,r.si19,r.si20, ";
				...
				$sql.="r.k1,r.k2,r.k3,r.k4,r.k5,r.k6,r.k7,r.k8,r.k9,r.k10,r.k11,r.k12,r.k13,r.k14,r.k15,r.k16,r.k17,r.k18,r.k19,r.k20, ";
				$sql.="r.k21,r.k22,r.k23,r.k24,r.k25,r.k26,r.k27,r.k28,r.k29,r.k30,r.k31,r.stok_akhir,(r.hpp*r.stok_akhir),(r.hpj*r.stok_akhir) ";
			$sql.="	FROM $table_rekap_stok r $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%' $tambahan and r.periode like '$filter_periode%'  ";
			$sql.=" AND r.kode_outlet like 'M0%' and length(trim(r.kode_outlet))=10  ";
			$sql.="	and ( r.barcode_15 like '$barcode%') $filter_minus  $sql_area  $sql_tbhn2 order by r.kode_outlet limit $jumlah_data; -- akses 2";
				...
				$sql.=" SUM(r.k21),SUM(r.k22),SUM(r.k23),SUM(r.k24),SUM(r.k25),SUM(r.k26),SUM(r.k27),SUM(r.k28),SUM(r.k29),SUM(r.k30),SUM(r.k31), ";
				$sql.=" sum(r.stok_akhir),sum(r.hpp*r.stok_akhir),sum(r.hpj*r.stok_akhir)  ";
				$sql.="FROM $table_rekap_stok r  $sql_inner_produk WHERE r.kode_outlet like '$txtoutlet%' $tambahan  and r.periode like '$filter_periode%' $sql_area ";
				$sql.=" AND r.kode_outlet like 'M0%' and length(trim(r.kode_outlet))=10  $sql_tbhn2 ";
				$sql.="GROUP BY r.kode_outlet,DATE_FORMAT(r.periode,'%Y-%m') ORDER BY r.kode_outlet LIMIT ".($page*$jmlHal).",".$jmlHal." -- start";	
		}
        unset($_SESSION['txtoutlet']);  
   ...
   # echo "<!-- SQL $sql -->";
    ...
    while ( list($periode,$tgl_stok_awal,$kode_outlet,$nama_outlet,$barcode_13,$barcode_15,$nama,$hpp,$hpj,$stok_awal,
	$si1,$si2,$si3,$si4,$si5,$si6,$si7,$si8,$si9,$si10,$si11,$si12,$si13,$si14,$si15,$si16,$si17,$si18,$si19,$si20,
	$si21,$si22,$si23,$si24,$si25,$si26,$si27,$si28,$si29,$si30,$si31,
	$ro1,$ro2,$ro3,$ro4,$ro5,$ro6,$ro7,$ro8,$ro9,$ro10,$ro11,$ro12,$ro13,$ro14,$ro15,$ro16,$ro17,$ro18,$ro19,$ro20,
	$ro21,$ro22,$ro23,$ro24,$ro25,$ro26,$ro27,$ro28,$ro29,$ro30,$ro31,
	...
	$k21,$k22,$k23,$k24,$k25,$k26,$k27,$k28,$k29,$k30,$k31,$stok_akhir,$stok_akhir_hpp,$stok_akhir_hpj)=mysql_fetch_array($hsltemp)){
	 ...
	 if($username=='budi-it'){
	   ...
	   if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)||!empty($txtprodukpilihan)){
		    $sql_sa="SELECT $sql_cache DATE_FORMAT(tanggal,'%d %M %Y') FROM stok_awal_reshare WHERE kode_outlet='$kode_outlet' limit 1";
			  list($tgl_stok_awal)=mysql_fetch_array(mysql_query($sql_sa));
	   }else{
		 if($detail<>1){
		      $sql_sa="SELECT $sql_cache DATE_FORMAT(tanggal,'%d %M %Y') FROM stok_awal_reshare WHERE kode_outlet='$kode_outlet' limit 1";
			  list($tgl_stok_awal)=mysql_fetch_array(mysql_query($sql_sa));
			  $barcode_13='';$barcode_15='';$nama='';
		 }
	   }  
...
?>
 ...
 if($export_to_excel=='1'){
    ...
    <tr onMouseOver="this.bgColor = '#CCCC00'" onMouseOut ="this.bgColor = '<?php echo $bgcolor;?>'" bgcolor="<?php echo $bgcolor; ?>" 
    class="datarow" id="<?php echo $kode_outlet.'_'. $barcode_15.'_'.$barcode_13; ?>">
 <?php }?>
    ...
    <td width="50">&nbsp;<?php echo $kode_outlet; ?></td>
    <td width="93">&nbsp;<?php echo $arrayReshare[$kode_outlet]; ?></td>
    ...
    <td width="93" class="detail1">&nbsp;<?php echo $nama;?></td>
     <td width="51" align="right" id="stokawal_<?php echo $kode_outlet.'_'. $barcode_15; ?>">
    <?php
	   if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
		   echo number_format($stok_awal);
	   }else{
		   echo "<a href=\"stok_outlet_periode.php?a=1&p=$filter_periode&r=$kode_outlet&n=$txt_nama\" target=\"_blank\">".number_format($stok_awal)."</a>"; 
	   }
	?>  
    </td>
    ...
    <?php 
	   for($i=1;$i<=$jpb;$i++){
	      $var_baru="\$si$i";
		  if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			  eval("echo \"<td align='right' $bgc_si class='detail' id='si_".$kode_outlet."_".$barcode_15."_".$i."'>$var_baru</td>\";");
		  }else{
			  eval("echo \"<td align='right' $bgc_si class='detail' id='si_".$kode_outlet."_".$barcode_15."_".$i."'><a href='rekap_stok_detail.php?o=$kode_outlet&p=$filter_periode&j=si$i&t=o' target='_blank'>$var_baru</a></td>\";");
		  }
	    ...
	    $tsi_total+=$tsi;
	?>
    <td align="right" bgcolor="#FF0099" class="total"><?php echo $tsi?></td>
    <?php 
	   for($i=1;$i<=$jpb;$i++){
	      $var_baru="\$ro$i";
		  //eval("echo \"<td align='right' class='detail'>$var_baru</td>\";");
		  if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			  eval("echo \"<td align='right' $bgc_ro class='detail' id='ro_".$kode_outlet."_".$barcode_15."_".$i."'>$var_baru</td>\";");
		  }else{
		  	eval("echo \"<td align='right' $bgc_ro class='detail' id='ro_".$kode_outlet."_".$barcode_15."_".$i."'><a href='rekap_stok_detail.php?o=$kode_outlet&p=$filter_periode&j=ro$i&t=o' target='_blank'>$var_baru</a></td>\";");
		  }
		  //echo("echo \"<td align='right' class='detail'>$var_baru</td>\";");		  
	   }
	   $tro_total+=$tro;	   
	?>
    <td align="right" bgcolor="#FF0099" class="total"><?php echo $tro?></td>
    <?php 
	   for($i=1;$i<=$jpb;$i++){
	      $var_baru="\$so$i";
		  if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			  eval("echo \"<td align='right' $bgc_so class='detail' id='so_".$kode_outlet."_".$barcode_15."_".$i."'>$var_baru</td>\";");
		  }else{
		  	eval("echo \"<td align='right' $bgc_so class='detail' id='so_".$kode_outlet."_".$barcode_15."_".$i."'><a href='rekap_stok_detail.php?o=$kode_outlet&p=$filter_periode&j=so$i&t=o' target='_blank'>$var_baru</a></td>\";");
		  }
		  //echo("echo \"<td align='right' class='detail'>$var_baru</td>\";");		  
	   }
	   $tso_total+=$tso;	   
	?>
    <td align="right" bgcolor="#FF0099" class="total"><?php echo $tso?></td>
    <?php 
	   for($i=1;$i<=$jpb;$i++){
	      $var_baru="\$ri$i";
		  if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			  eval("echo \"<td align='right' $bgc_ri class='detail' id='ri_".$kode_outlet."_".$barcode_15."_".$i."'>$var_baru</td>\";");
		  }else{
		  //eval("echo \"<td align='right' class='detail'>$var_baru</td>\";");
		  		eval("echo \"<td align='right' $bgc_ri  class='detail' id='ri_".$kode_outlet."_".$barcode_15."_".$i."'><a href='rekap_stok_detail.php?o=$kode_outlet&p=$filter_periode&j=ri$i&t=o' target='_blank'>$var_baru</a></td>\";");
		  }
		  //echo("echo \"<td align='right' class='detail'>$var_baru</td>\";");		  
	   }	
	   $tri_total+=$tri;   
	?>
    <td align="right" bgcolor="#FF0099" class="total"><?php echo $tri?></td>
    <?php 
	   for($i=1;$i<=$jpb;$i++){
	      $var_baru="\$k$i";
		  if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
			  eval("echo \"<td align='right' $bgc_k class='detail' id='k_".$kode_outlet."_".$barcode_15."_".$i."'>$var_baru</td>\";");
		  }else{
		  //eval("echo \"<td align='right' class='detail'>$var_baru</td>\";");
		  	eval("echo \"<td align='right' $bgc_k class='detail' id='k_".$kode_outlet."_".$barcode_15."_".$i."'><a href='rekap_stok_detail.php?o=$kode_outlet&p=$filter_periode&j=k$i&t=o' target='_blank'>$var_baru</a></td>\";");
		  }
		  //echo("echo \"<td align='right' class='detail'>$var_baru</td>\";");		  
	   }	 
	   $tk_total+=$tk;  
	?>
    <td align="right" bgcolor="#FF0099" class="total"><?php echo $tk?></td>
    <?php
	...
	?>
    <?php if($stok_in>0){$bg1='bgcolor="#3399FF"';}else{$bg1='';}?>
    <?php if($retur_out>0){$bg2='bgcolor="#3399FF"';}else{$bg2='';}?>
    <?php if($stok_out>0){$bg3='bgcolor="#3399FF"';}else{$bg3='';}?>
    <?php if($retur_in>0){$bg4='bgcolor="#3399FF"';}else{$bg4='';}?>
    <td width="51" align="right">
    <?php
	   if((isset($cari_barcode)&& ( !empty($barcode) ||!empty($txt_nama)))||isset($cari_minus)){
		   $hr=date('j');
		   echo "<div class='edit_text' id='$hr#$kode_outlet#$filter_periode#$barcode_13#$barcode_15'>".number_format($stok_akhir)."</div>"; 
	   }else{
		   echo "<a href=\"stok_outlet_periode.php?a=0&p=$filter_periode&r=$kode_outlet&n=$txt_nama\" target=\"_blank\" id='stok_".$kode_outlet."_".$barcode_13."_".$barcode_15."'>".number_format($stok_akhir)."</a>"; 
	   }
	?>  
    </td>
    ...
    <?php
	   for($i=1;$i<=$hari_ini;$i++){
	       echo "<td align='center'  id='stok_".$kode_outlet."_".$barcode_13."_".$barcode_15."_".$i."' class='stok'>0</td>"; 
	   }
...
<tr>
                        <td class="text_standard">
                            ...
                            <?php for($i=0;$i<($jmlData[0]/$jmlHal);$i++){ 
                                if($hal<=0){ ?>
                                    <span class="<?php if($i==$hal) echo "hal_select"; else echo "hal"; ?>" onclick="location.href='monitoring_rekap_stok_detail_with_actual.php?hal=<?php echo $i; ?><?php echo $tambah?>';"><?php echo ($i+1); ?></span>
                                    <?php if($i>=4) break;
